...
def main():
    ...
    logger.info(
        f"Global Average Salary: S/. {global_avg:.2f} (from {len(global_salaries)} valid salaries)"
    )
    ...
    for cluster in response.data:
        ...
        supabase.table("clusters").update({"market_insights": market_insights}).eq(
            "cluster_id", c_id
        ).execute()
        logger.info(
            f"Updated {cluster['name']} -> Avg: S/.{c_avg:.2f} ({diff_percentage:+.1f}%), Share: {market_share:.1f}%"
        )
