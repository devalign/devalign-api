    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "messages": [
                    {"role": "system", "content": "You are a JSON returning bot. Output only valid JSON."},
        ...
        if response.status_code == 200:
            ...
            for skill in skills:
                ...
                if domain in DOMAINS:
                    supabase.table("skills").update({"domain": domain}).eq("skill_id", skill["skill_id"]).execute()
                else:
                    # Fallback mapping if LLM fails
                    supabase.table("skills").update({"domain": "Backend"}).eq("skill_id", skill["skill_id"]).execute()
