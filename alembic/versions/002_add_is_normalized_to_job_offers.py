...
down_revision: str | None = "001_create_all_tables"
...
def upgrade() -> None:
    ...
    op.add_column(
        "job_offers",
        sa.Column("is_normalized", sa.Boolean(), server_default="false", nullable=False),
    )
    # Create an index on it since we'll be querying WHERE is_normalized = False
    op.create_index(
        op.f("ix_job_offers_is_normalized"), "job_offers", ["is_normalized"], unique=False
    )
